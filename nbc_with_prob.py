...
def predict(pos_prob, neg_prob, pos_likelihood, neg_likelihood, test_data):
    ...
    for sample in test_data:
        ...
        p =  math.exp(pos_score) 
        n = math.exp(neg_score)
        t = p + n
        if  pos_score >= neg_score:
            results.append((n / t, p / t, 1))
        else:
            results.append((n / t, p / t, 0))
    return results


def square_loss(pred, actual):
    # now pred is a list of tuples;
    # (neg_prediction_probability, pos_prediction_probability, label)
    n = len(actual)
...
def main():
    ...
    print square_loss(prediction, test_Y)
